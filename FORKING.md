...
| EAS Ping watcher constructor visibility — `EasPingWatcher::new` widened `pub(crate)`→`pub` with a host-seam doc line (the type was already re-exported; no behavior change) (1 commit) | kylins' scheduler drives its own EAS push loop (mirroring its IMAP IDLE arm) and the watcher's `EasClient` internals are engine-private — the orphan rule leaves no host-side construction seam |

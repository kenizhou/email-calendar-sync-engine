...
| Outbox release-on-retryable — `settle_outcome` becomes the single mark-or-release decision shared by the inline drivers' `record_failure`/`record_send_failure` and every drainer's `settle_claimed`: a `Failed` classified `Retryable`/`RateLimited`/`NeedsResync` releases the op back to `Pending` (the T7b release verb) instead of recording a terminal mark, while `Permanent`/`Conflict`/`Authentication` stay terminal and `NeedsConfirmation` stays parked — the retry the failure classification always promised (a lifted rate limit, a sync pass that warms a cold adapter's SyncKey) is now the drainer's to deliver; release-then-replay tests for a rate-limited submit and a cold EAS calendar write (1 commit, `a87c36e`) | kylins T13 live acceptance surfaced it: the one-shot CLI create refused NeedsResync was recorded terminal, so the error hint's "the outbox retries the write after it" never happened — found by driving the real account; spec: same p2 spec, task 13 |
